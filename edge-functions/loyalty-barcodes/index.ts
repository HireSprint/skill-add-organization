...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (size > MAX_BYTES) return json({ error: "file_too_large", max_bytes: MAX_BYTES }, 413);
    ...
    if (event_id) {
      const { error: updateEventErr } = await supabaseAdmin.from("events").update({ event_image: fullUrl }).eq("id", event_id);
      if (updateEventErr) console.error("DB update (events) error:", updateEventErr);
    }
    if (loyalty_card_id) {
      const { error: updateLoyaltyErr } = await supabaseAdmin.from("loyalty_cards").update({ barcode_url: fullUrl }).eq("user_id", userIdFromToken).eq("card_number", loyalty_card_id);
