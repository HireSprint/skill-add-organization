...
async function sendExpoNotification(shopList, pushTokens) {
  ...
  const messages = validTokens.map((token) => ({
    ...
    body: `Your shopping list "${shopList.title}" is ${shopList.status} — scheduled for ${shopList.delivery_date} at ${shopList.delivery_time}. Don't miss out!`,
    data: { shopListId: shopList.id, title: shopList.title, content: shopList.content, status: shopList.status, screen: "events" }
  }));
  const chunkSize = 100;
  const tickets = [];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: shopLData, error: shopLDataErr } = await supabase.from("shopping_lists").update({ status }).eq("id", id).select("*").single();
    if (shopLDataErr || !shopLData) {
      return new Response(JSON.stringify({ error: "Shopping List not found", details: shopLDataErr?.message }), {
        status: 404,
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
    const { data: users, error: usersErr } = await supabase.from("profiles").select("push_token").eq("id", user_id).not("push_token", "is", null).neq("push_token", "");
    if (usersErr) throw usersErr;
    const pushTokens = users.map((u) => u.push_token).filter(Boolean);
    await sendExpoNotification(shopLData, pushTokens);
    return new Response(JSON.stringify({ success: true, shoppingListId: id, notified_users: pushTokens.length }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" }
    });
  } catch (err) {
